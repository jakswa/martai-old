...
var station_name_mapping = {
  ...
  'EDGEWOOD-CANDLER PARK STATION': 'EDGEWOOD STATION',
    ...
    stop_data.split('\r\n').forEach(function(s) {
      ...
      if (stop_ids[stop_id]) {
        ...
        _.each(trips, function(trip, trip_id) {
          trip.stops = _.sortBy(trip.stops, 'time');
          for (var i = 0; i < trip.stops.length; i++) {
            ...
            var ind = _.indexOf(service[trip.dir][trip.line], function(t) {
              return t > trip_stop.time;
            });
